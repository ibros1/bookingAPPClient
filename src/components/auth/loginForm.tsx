...
import { loginFn, handleGoogleCallback } from "@/redux/slices/users/auth/login";
...
const LoginForm: React.FC = () => {
  ...
  const loginReturnUrl = useMemo(() => `${window.location.origin}/login`, []);
  ...
  useEffect(() => {
    ...
    if (savedUserData) {
      try {
        const { user, token, refreshToken } = JSON.parse(savedUserData);
        dispatch(handleGoogleCallback({ user, token, refreshToken }));
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (token && refreshToken && userParam) {
      try {
        ...
        const userData = { user: decodedUser, token, refreshToken };
        localStorage.setItem("user_data", JSON.stringify(userData));

        dispatch(handleGoogleCallback(userData));
        navigate("/dashboard");
      } catch (err) {
      ...
      <div className="w-[95%] md:w-[60%] lg:w-[40%] xl:w-[32%] pb-8 pt-4 mb-16 mt-6 bg-white dark:bg-[#111A33] rounded-lg shadow-md border border-gray-200 dark:border-gray-700 transition-all duration-300">
        ...
        <CardContent>
          ...
          >
            ...
            >
              {logInState.loading || formik.isSubmitting ? (
